  $(document).ready(function () {
    ...
    var isM = (ww<780);
      ...
      $('.addDate').click(function (event) {
        ...
        if (isM){
            var urlForm = '/admin/citas-nutricion-getafe/create/'+dateForm+'/'+timeForm;
            if (typeCalend == 'week') urlForm += '?weekly';
            window.location.href = urlForm;
        } else {
            $('#ifrModal').attr('src', '/admin/citas-nutricion-getafe/create/' + dateForm + '/' + timeForm);
            $('#modalIfrm').modal();
        }
      });
      $('.editDate').on('click', '.events', function (event) {
        ...
        var id = $(this).data('id');
         if (isM){
             var urlForm = '/admin/citas-nutricion-getafe/edit/' + id;
            if (typeCalend == 'week') urlForm += '?weekly';
            window.location.href = urlForm;
        } else {
            $('#ifrModal').attr('src', '/admin/citas-nutricion-getafe/edit/' + id);
            $('#modalIfrm').modal();
        }
      });
      $('.selectDate').on('click', 'li', function (event) {
        ...
        location.assign("/admin/citas-nutricion-getafe/" + val + "/" + coach + "/" + type);
      });
      $('.coachsFilter').on('click', 'li', function (event) {
        ...
        if (typeCalend == 'week'){
            location.assign("/admin/citas-nutricion-getafe-week/" + week + "/" + coach + "/" + type);
        } else {
            location.assign("/admin/citas-nutricion-getafe/" + month + "/" + coach + "/" + type);
        }
      });
      ...
      $('#modal_newUser').on('submit', '#form-new', function (event) {
        ...
        var posting = $.post(url, $form.serialize()).done(function (data) {
          if (data == 'OK') {
            $('#content-add-date').load('/admin/citas-nutricion-getafe/create/' + dateForm + '/' + timeForm);
      ...
      //    
        ...
        $('.btn-bloqueo').click(function (e) {
          e.preventDefault();
          $('#ifrModal').attr('src','/admin/citas/bloqueo-horarios/nutriG');
        });
