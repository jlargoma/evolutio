...
//   isM = true;
    ...
    $('.addDate').click(function(event){
        ...
        if (isM){
            var urlForm = '/admin/citas-estetica/create/'+dateForm+'/'+timeForm;
            if (typeCalend == 'week') urlForm += '?weekly';
            window.location.href = urlForm;
        } else {
        $('#ifrModal').attr('src','/admin/citas-estetica/create/'+dateForm+'/'+timeForm);
        $('#modalIfrm').modal();
        }
    });
    $('.editDate').on('click','.events',function(event){
        ...
        var id = $(this).data('id');
        if (isM){
            var urlForm = '/admin/citas-estetica/edit/' + id;
            if (typeCalend == 'week') urlForm += '?weekly';
            window.location.href = urlForm;
        } else {
        $('#ifrModal').attr('src','/admin/citas-estetica/edit/'+id);
        $('#modalIfrm').modal();
        }
    });
    ...
    $('.selectDate').on('click','li',function(event){
        ...
        location.assign("/admin/citas-estetica/"+val+"/"+coach+"/"+type);
    });
    $('.coachsFilter').on('click','li',function(event){
        ...
        if (typeCalend == 'week'){
            location.assign("/admin/citas-estetica-week/" + week + "/" + coach + "/" + type);
        } else {
            location.assign("/admin/citas-estetica/" + month + "/" + coach + "/" + type);
        }
...
//            if (data == 'OK'){
//                if (isM){
//                    window.location.href = '/admin/citas-estetica/create/'+dateForm+'/'+timeForm;
//                } else {
//           
//                $('#content-add-date').load('/admin/citas-estetica/create/'+dateForm+'/'+timeForm);
...
//    });
  ...
  $('.btn-bloqueo').click(function (e) {
    e.preventDefault();
    $('#ifrModal').attr('src','/admin/citas/bloqueo-horarios/esthetic');
  });
