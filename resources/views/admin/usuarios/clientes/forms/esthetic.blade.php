...
<div class="row">
  <div class="col-md-8">
    <div class="boxFile">
      ...
      @if(isset($filesEsthetic) && count($filesEsthetic) )
      ...
      <table class="table">
        @foreach($filesEsthetic as $k=>$v)
        <tr>
          <th style="width: 80%;">{{$v['name']}}</th>
          <td>
            <button class="btn btn-danger delFileEsthetic" title="Borrar Archivo" data-k="{{$k}}"><i class="fa fa-trash"></i></button>
          </td>
          <td>
...
<script>
  $('.delFileEsthetic').on('click', function (e) {
