...
<p class="alert alert-warning  text-center"><b>Importante: </b> El eliminar un cobro no modifica la cantidad de bonos del usuario</p>
@endif
<?php
$price =  $oBono->price;
?>
...
<form class="fomr-horizontal content-md" method="post" action="{{ url('/admin/cobros/cobrar/' . $charge->id) }}" id="forms">
  ...
  <input type="hidden" id="id_tax" name="id_tax" class="form-control" value="<?php echo $id_tax; ?>" />
  <div class="row">
    <div class="col-md-4">
      <label for="type_payment">Forma de pago</label>
      <select class="form-control" name="type_payment" id="type_payment" <?php if ($disableType) echo 'disabled'; ?>>
    ...
    <div class="col-md-3">
      ...
      <select class="form-control" id="id_coach" name="id_coach" style="width: 100%; cursor: pointer" placeholder="Personal asignado" required="">
        <option></option>
        <?php
        $old = old('id_coach');
        foreach ($coachs as $v) :
