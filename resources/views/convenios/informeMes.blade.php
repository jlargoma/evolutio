...
<div class="content content-boxed bg-gray-lighter">
  ...
  <div class="col-xs-12 btn-months mx-1em">
    ...
    <a href="<?= '/admin/convenios/informes/'.$k.( ($convenio) ? '/'.$convenio : '' ) ?>" class=" btn btn-success <?php echo ($month == $k) ? 'active' : '' ?>">
      {{$v.' '.$year}}
    </a>
    @endforeach
  </div>
...
<script type="text/javascript">
  $('#filter_form').on('click', function(event) {
    ...
    window.location.href = '/admin/convenios/informes/' + month + '/' + convenio;
  });
  ...
  $('#searchInform').keydown(function(evt) {
    setTimeout(function() {
      ...
      var month = $('#month').val();
      var convenio = $('#convenio').val();
      $.post('/admin/convenios/informes/' + month + '/' + convenio, {
        search: search,
        _token: token
      }).done(function(data) {
